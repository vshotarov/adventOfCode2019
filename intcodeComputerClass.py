class IntcodeComputer:
    def __init__(self, code="", inputCallback=None, inputs=[],
            outputCallback=None):
        self.setCode(code)
        self.reset()
        self.setInputCallback(inputCallback)
        self.setOutputCallback(outputCallback)
        self.inputs = inputs

    def setInputCallback(self, inputCallback):
        self.inputCallback = inputCallback

    def setOutputCallback(self, outputCallback):
    ...
    def getState(self):
        return (list(self.code), self.pointer, self.relativeBase, self.finished)

    def setState(self, code, pointer, relativeBase, finished):
        self.code = list(code)
    ...
    def step(self, _input=None):
        op = self.code[self.pointer] % 100

        if op == 1:
            #print("Evalutaing addition at", self.pointer)
            # Add
            self.code[self._getAddress(3)] = self._getArg(1) + self._getArg(2)
            self.pointer += 4
        elif op == 2:
            #print("Evalutaing mult at", self.pointer)
            # Multiply
            self.code[self._getAddress(3)] = self._getArg(1) * self._getArg(2)
            self.pointer += 4
        elif op == 3:
            #print("Evalutaing input at", self.pointer)
            # Take input
            if _input is not None:
                self.code[self._getAddress(1)] = _input
            elif self.inputs:
                self.code[self._getAddress(1)] = self.inputs.pop(0)
            elif self.inputCallback:
                self.code[self._getAddress(1)] = self.inputCallback()
            else:
                self.code[self._getAddress(1)] = int(input("Input: "))
            self.pointer += 2
        elif op == 4:
            #print("Evalutaing output at", self.pointer)
            # Output
            out = self._getArg(1)
            if self.outputCallback:
                self.outputCallback(out)
            self.pointer += 2
            return out
        elif op == 5:
            #print("Evalutaing jumpIfTrue at", self.pointer)
            # Jump if true
            if self._getArg(1) != 0:
                self.pointer = self._getArg(2)
            else:
                self.pointer += 3
        elif op == 6:
            #print("Evalutaing jumpIfFalse at", self.pointer)
            # Jump if false
            if self._getArg(1) == 0:
                self.pointer = self._getArg(2)
            else:
            ...
            #print("Evalutaing lessThan at", self.pointer)
            # Less than
            self.code[self._getAddress(3)] = 1 if\
                    self._getArg(1) < self._getArg(2) else 0
            self.pointer += 4
        elif op == 8:
            #print("Evalutaing equals at", self.pointer)
            # Equals
            self.code[self._getAddress(3)] = 1 if\
                    self._getArg(1) == self._getArg(2) else 0
            self.pointer += 4
        elif op == 9:
            #print("Evalutaing offsetRelativeBase at", self.pointer)
            # Offset relative base
            self.relativeBase += self._getArg(1)
            self.pointer += 2
        elif op == 99:
            #print("Evalutaing exit at", self.pointer)
            self.pointer += 1
            self.finished = True
        else:
