def createGrid(size=20):
    return [["." for i in range(size)] for j in range(size)]

def printGrid(grid):
    print "\n".join(["".join(g) for g in grid])

def isPointOnLine(pt, line):
    lineRangeX = sorted([line[0], line[2]])
    lineRangeY = sorted([line[1], line[3]])

    if not (pt[0] >= lineRangeX[0] and pt[0] <= lineRangeX[1]):
        return False

    if not (pt[1] >= lineRangeY[0] and pt[1] <= lineRangeY[1]):
        return False
...
def linesIntersect(a,b):
    ...
    if a[0] < bX[0] and a[2] < bX[0]:
        # Both A's X components are on the same side of B's X
        return False

    if a[0] > bX[1] and a[2] > bX[1]:
        # Both A's X components are on the same side of B's X
        return False

    if a[1] < bY[0] and a[3] < bY[0]:
        # Both A's Y components are on the same side of B's Y
        return False

    if a[1] > bY[1] and a[3] > bY[1]:
        # Both A's Y components are on the same side of B's Y
        return False

    # Clip A by B
    intersection[0] = max(min(intersection[0], bX[1]), bX[0]) # Clip A's first point X
    intersection[2] = max(min(intersection[2], bX[1]), bX[0]) # Clip A's second point X
    intersection[1] = max(min(intersection[1], bY[1]), bY[0]) # Clip A's first point Y
    intersection[3] = max(min(intersection[3], bY[1]), bY[0]) # Clip A's second point Y
...
L1009,D700,L634,U294,R898,D947,R650,U988,L623,D968,R761,U490,R525,U76,R633,D139,R348,D855,L983,U553,L454,D211,L240,D465,R260,U285,R653,D734,L346,U434,R813,U599,R98,D779,L58,D6,R309,U437,L712,U896,R262,U911,R400,D247,R297,U915,L223,D591,L755,D398,L980,U177,R186,U882,R418,U763,L741,D60,L942,U648,L430,D401,R30,D157,L901,D179,L615,U535,L586,D613,L606,U239,L133,D251,L549,D579,R612,U307,L127,D343,L43,D288,R245,U122,R352,D527,R476,U24,L805,U291,R953,D469,L941,U577,L384,U345,L463,D50,R311,D649,L746,D902,R644,U913,R147,D649,R848,D673,R93,U65,R363,U734,L289,U599,R738,U45,R128,D508,L93,D201,R51,U239,R17,D496,L661,D912,R165,U291,L207,D308,R241,D388,L910,D821,R714,D327,L605,U880,L682,D934,R334,D1,R602,D54,L51,D913,L575,U168,R614,D603,R452,U718,R689,D505,R83,D385,R636,D692,R424,D573,R686,D572,L467,D698,L21,U510,L497,U329,R286,U733,R584,U919,R499,U971,L558,U511,L565,D623,L502,U536,L483,U372,L686,D420,L900,U316,L37,U372,L915,D641,R165,D927,L137,U231,R813,U416,L131,D530,R486,U795,L507,U757,L208,U308,L521,U583,L758,U654,R554,D467,R381,U155,R47,U829,R92,D158,R54,D500,R17,D471,R748,U571,L194,D55,L921,U271,L730,U207,L204,U806,R19,D33,R218,D911,L106,U220,R551,U308,L268,U5,L374,U657,R639,U705,R294,U962,L927,U892,L477,U290,R378,D193,L154,U859,L618,D690,L769,D779,R752,D915,L693,D586,L558,D864,L523,U354,R386,U236,R888,U302,L75,U628,R132,D306,L939,U73,L687,D488,R21,D760,L856,U96,L116,U557,L639,U812,L389,D364,L807,U696,R781,D625,R565,U728,R134,D406,R785,U583,R60,D819,L939"""
    ...
    wirePositions = [[0,0],[0,0]]
    wires = [[],[]]

    for wire, wirePos, line in zip(wires, wirePositions, _input.splitlines()):
        for instruction in line.split(","):
            ...
            wire.append([wirePos[0], wirePos[1],
                wirePos[0]+step[0]*steps, wirePos[1]+step[1]*steps])

            wirePos[0] += step[0]*steps
            wirePos[1] += step[1]*steps

    manhattanDistancesToIntersections = []
    distancesAlongWiresToIntersections = []
    distanceTraveledOnA = 0
    for lineA in wires[0]:
        distanceTraveledOnB = 0

        intersection = False
        for lineB in wires[1]:
            intersection = linesIntersect(lineA, lineB)

            if intersection:
                manhattanDistancesToIntersections.append(
                        abs(intersection[0]) + abs(intersection[1]))

                distanceToIntersectionFromA = abs(intersection[0] - lineA[0]) +\
                        abs(intersection[1] - lineA[1])
                distanceToIntersectionFromB = abs(intersection[0] - lineB[0]) +\
                        abs(intersection[1] - lineB[1])

                distancesAlongWiresToIntersections.append(distanceTraveledOnA +\
                        distanceToIntersectionFromA +\
                        distanceTraveledOnB + distanceToIntersectionFromB)

            distanceTraveledOnB += abs(lineB[2] - lineB[0]) + abs(lineB[3] - lineB[1])

        distanceTraveledOnA += abs(lineA[2] - lineA[0]) + abs(lineA[3] - lineA[1])

    print "Manhattan distance to closest intersection (part 1) is %i" %\
        sorted(manhattanDistancesToIntersections)[1]
    print "Fewest combined steps along wires to reach an intersection (part 2) is %i" %\
        sorted(distancesAlongWiresToIntersections)[1]
